...
async function trackAICost(userId: string, operation: string, response: any): Promise<void> {
...
${userContext.recentActivity
  .slice(0, 3)
  .map((cmd) => `- ${cmd.intent} (${cmd.timestamp.toLocaleDateString()})`)
  .join('\n')}

Frequent Contacts:
${userContext.frequentContacts
  .slice(0, 5)
...
- Always check calendar before proposing meeting times`;
  ...
  private calculateConfidence(message: any, toolCalls: ToolCall[]): number {
...
describe('IntentClassifierService', () => {
  it('should classify meeting scheduling intent', async () => {
    const mockGPT = jest.spyOn(openai.chat.completions, 'create').mockResolvedValue({
      choices: [
        {
          message: {
            tool_calls: [
