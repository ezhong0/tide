...
const logger = {
    debug: (...args) => console.debug('[Retry]', ...args),
...
export async function retryWithBackoff(fn, options) {
    const { maxAttempts, initialDelayMs, maxDelayMs, backoffMultiplier, retryableErrors, isRetryable, name = 'Operation', } = options;
    let lastError;
    ...
    throw new RetryExhaustedError(`${name} failed after ${maxAttempts} attempts: ${lastError.message}`, maxAttempts, lastError);
}
/**
 * Check if an error is retryable
 */
function isErrorRetryable(error, retryablePatterns, customCheck) {
 ...
 */
export function wrapWithRetry(fn, options) {
    return (async (...args) => {
