...
export class CalendarOptimizer {
     ...
     */
    calculateFragmentedTime(events) {
     ...
     * Count focus blocks (2+ hour uninterrupted periods)
     */
    countFocusBlocks(events) {
        // Define working hours (9 AM - 6 PM)
        const workStart = 9;
        const workEnd = 18;
        // Get all days that have events
        const days = new Set(events.map((e) => e.start.toISOString().split('T')[0]));
     ...
     */
    createOptimizationActions(opportunities) {
     ...
     */
    calculateProjectedImpact(opportunities) {
        const timeRecovered = opportunities.reduce((sum, opp) => sum + (opp.timeRecovered || 0), 0);
        const focusTimeCreated = opportunities
            .filter((opp) => opp.type === 'create_focus_block')
            .length * 120; // 2 hours per focus block
        const meetingsReduced = opportunities.filter((opp) => opp.type === 'skip_meeting').length;
        const fragmentationReduced = opportunities.some((opp) => opp.type === 'consolidate_meetings')
            ? 50
            : 0; // Percentage
        return {
            timeRecovered,
