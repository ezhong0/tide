 ...
 */
export class ExchangeCalendarProvider implements ICalendarProvider {
  ...
  async fetchEvents(start: Date, end: Date): Promise<CalendarEvent[]> {
    ...
    try {
      ...
      return response.value
        .map((event: Record<string, any>) => this.transformToCalendarEvent(event))
   ...
   */
  private transformToCalendarEvent(event: Record<string, any>): CalendarEvent | null {
    ...
    const attendees: Attendee[] = (event.attendees || []).map((a: any) => ({
   ...
   */
  private formatRecurrenceRule(recurrence: any): string {
