...
export class ReasoningVerifier {
     ...
     */
    async verify(result, modelClient) {
     ...
     */
    async checkLogicalConsistency(result) {
        const { output } = result;
     ...
     */
    async checkFactualAccuracy(result, modelClient) {
...
${claims.join('\n')}`;
     ...
     */
    async detectHallucination(result, modelClient) {
        const claims = result.claims || [];
        if (claims.length === 0) {
            return {
...
//# sourceMappingURL=reasoning-verifier.js.map