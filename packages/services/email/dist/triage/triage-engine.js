...
class EmailTriageEngine {
    ...
    async analyzeImportance(email) {
        ...
        factors.subjectKeywords = importantKeywords.some((regex) => regex.test(email.subject))
            ? 0.3
            : 0;
        // Money mentioned
    ...
    async analyzeUrgency(email) {
        ...
        const hasUrgentMarker = urgentMarkers.some((regex) => regex.test(email.subject) || regex.test(email.body));
        if (hasUrgentMarker) {
            return 'immediate';
        }
    ...
    async categorizeEmail(email) {
        ...
        // Meeting patterns
        if (/meeting|call|discuss|sync|catch up|1:1|one.on.one/i.test(subject + body) ||
            /calendar|schedule|availability/i.test(body)) {
            return 'meeting';
        }
        // Request patterns
        if (/can you|could you|please|need|require|request/i.test(subject + body) ||
            /\?/.test(subject)) {
            return 'request';
        }
        // Newsletter patterns
        if (/newsletter|digest|roundup|weekly|monthly/i.test(subject) ||
            email.from.includes('noreply') ||
            email.from.includes('no-reply')) {
            return 'newsletter';
        }
        // Social patterns
        if (email.from.includes('@facebook') ||
        ...
        if (/sale|discount|offer|promotion|deal|limited time/i.test(subject) ||
     ...
     */
    async detectActionRequired(email) {
        const text = (email.subject + ' ' + email.body).toLowerCase();
        // Schedule action
        if (/schedule|calendar|meeting|availability|when (are|can) you/i.test(text) ||
            /book (a )?time/i.test(text)) {
            return 'schedule';
        }
        // Reply action
        if (/\?/.test(email.subject + email.body) ||
        ...
        if (/fyi|for your information|just wanted to|keeping you informed/i.test(text) ||
            email.from.includes('noreply')) {
            return 'file';
        }
        return 'none';
     ...
     */
    async analyzeRelationships(email) {
    ...
    determineStrategy(context) {
        ...
        if (context.category === 'newsletter' ||
    ...
    calculateConfidence(context) {
        ...
        if (context.actionRequired === 'schedule' ||
