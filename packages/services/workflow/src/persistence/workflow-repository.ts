...
export class WorkflowRepository {
   ...
   * Get workflow by ID
   */
  async getWorkflow(id: WorkflowId): Promise<WorkflowDefinition | null> {
    const query = `
    ...
    try {
      ...
      const workflow: WorkflowDefinition = typeof row.definition === 'string'
   ...
   */
  async getWorkflowsByUser(userId: UserId): Promise<WorkflowDefinition[]> {
    const query = `
      SELECT * FROM tide.workflows
   ...
   */
  async saveExecution(execution: WorkflowState): Promise<void> {
    const query = `
   ...
   */
  async loadExecution(id: WorkflowExecutionId): Promise<WorkflowState | null> {
    const query = `
    ...
    try {
      ...
      const state: WorkflowState = {
        id: row.id as WorkflowExecutionId,
      ...
      if (state.context && state.context.stepResults) {
        if (Array.isArray(state.context.stepResults)) {
          state.context.stepResults = new Map(Object.entries(state.context.stepResults)) as any;
        } else if (typeof state.context.stepResults === 'object') {
          state.context.stepResults = new Map(Object.entries(state.context.stepResults as object)) as any;
        }
      }
   ...
   */
  async updateExecution(execution: WorkflowState): Promise<void> {
   ...
   * Delete execution
   */
  async deleteExecution(id: WorkflowExecutionId): Promise<void> {
    const query = `
   ...
   */
  async getExecutionsByWorkflow(workflowId: WorkflowId, limit = 100): Promise<WorkflowState[]> {
    const query = `
      SELECT * FROM tide.workflow_executions
      WHERE workflow_id = $1
      ORDER BY created_at DESC
   ...
   */
  async getActiveExecutions(): Promise<WorkflowState[]> {
    const query = `
      SELECT * FROM tide.workflow_executions
...
export class PostgreSQLStatePersistence {
  ...
  async load(id: string): Promise<WorkflowState | null> {
    return await this.repository.loadExecution(id as WorkflowExecutionId);
  }
  ...
  async delete(id: string): Promise<void> {
    await this.repository.deleteExecution(id as WorkflowExecutionId);
  }
}
